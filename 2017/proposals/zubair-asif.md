...
We will be following closely the SGHMC sampler as described in Chen *et al.* [8]. However, this implementation is not without its caveats and requires a number if user-defined quantities. To this end, the scale adapted stochastic gradient HMC sampler due to Springenberg *et al.* can be implemented. However, we must emphaisize novelty of these methods and especially [some debate on their correctness](http://andrewgelman.com/2016/12/09/fundamental-incompatibility-scalable-hamiltonian-monte-carlo-naive-data-subsampling/). Interestingly as well, at the time of writing this, there is a new [Pull Request](https://github.com/pymc-devs/pymc3/pull/1977) targeted at stochastic gradient approaches. However, it is important to emphasize that these are distinct approaches from the SGHMC appraoch. We'll ensure that efforts are not duplicated and both implementations inform each other. 
